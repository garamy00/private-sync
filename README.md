...
- `agent` (노트북): watchdog으로 변경을 감지해 3초 디바운스 후 rsync over SSH로 업로드한다.
  삭제는 전파하지 않는다. 데몬이 시작될 때 설정된 모든 대상을 한 번 동기화하므로, 처음
  설치했을 때와 노트북을 껐다 켠 뒤에도 서버가 최신 상태가 된다. 실제로 전송되는 것은
  rsync가 비교해 찾아낸 차이뿐이다.
...
`agent.yaml`을 저장하면 보통 몇 초 안에 자동으로 반영되므로 재시작이 필요 없다.
다만 노트북이 사내망 밖이라 업로드가 계속 실패하는 중이면 재시도 backoff 때문에
반영이 최대 5분까지 늦어질 수 있다. 설정에
...
- 폴더 화면의 `📦 이 폴더 통째로 받기` 버튼을 탭하면 먼저 파일 개수와 크기를 보여주고
  받기/취소를 묻는다. **받기**를 누르면 폴더 전체를 하나의 AES-256 암호 ZIP으로 묶어
  보내며, 같은 메시지가 `압축 중… → 전송 중… → 완료`로 갱신되어 진행 상황을 보여준다.
  큰 폴더는 파일 다운로드와 마찬가지로 `.partNN`으로 분할되어 오므로 PC에서
  `cat 폴더명.zip.part* > 폴더명.zip`으로 합쳐야 한다(로컬 API 서버를 쓰면 업로드 한도가
  커져 대개 분할 없이 한 파일로 온다).
